...
{
 	DBG_PRINT(F("Testing: "));
 	DBG_PRINT(TICke_LOW(desired), DEC);
 	DBG_PRINT(F(" <= "));
 	DBG_PRINT(measured, DEC);
 	DBG_PRINT(F(" <= "));
 	DBG_PRINT(TICke_HIGH(desired), DEC);

  bool passed = ((measured >= TICke_LOW(desired)) && (measured <= TICke_HIGH(desired)));
  if (passed)
...
int  MATCH_MARK (int measured_ticks,  int desired_us)
{
	...
	DBG_PRINT("us"); 
	DBG_PRINT(": ");
	DBG_PRINT(TICke_LOW(desired_us + MARK_EXCESS) * USECPERTICK, DEC);
	DBG_PRINT(F(" <= "));
	DBG_PRINT(measured_ticks * USECPERTICK, DEC);
	DBG_PRINT(F(" <= "));
	DBG_PRINT(TICke_HIGH(desired_us + MARK_EXCESS) * USECPERTICK, DEC);

  bool passed = ((measured_ticks >= TICke_LOW (desired_us + MARK_EXCESS))
                && (measured_ticks <= TICke_HIGH(desired_us + MARK_EXCESS)));
  if (passed)
    DBG_PRINTLN(F("?; passed"));
  else
    DBG_PRINTLN(F("?; FAILED")); 
